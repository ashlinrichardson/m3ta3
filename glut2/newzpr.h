...
public:
	...
	if(myGlutID_AsFunctionOfZprWindowID[zprInstanceID]!=currentID){
	    dprintf("Error: glutWindowID (%d) vs. myGlutID_AsFunctionOfZprWindowID[zprInstanceID] (%d) mismatch.\nmyZprWindowID_AsFunctionOfGlutID[currentID]=%d", currentID, myGlutID_AsFunctionOfZprWindowID[zprInstanceID],myZprWindowID_AsFunctionOfGlutID[currentID]); exit(1);
	}
...
private:
    ...
    zprManager(int argc, char *argv[]){
		  ...
		  int i;  for(i=0; i<MAX_ZPR_INSTANCES; i++){ (*myZprInstances)[i] = NULL;}
		  nextZprInstanceID = 0;
		  printf("glutInit()\n");
		  glutInit(&argc, argv);
		  printf("after glutInit()\n");	
    }
    void focus(int myID){	glutSetWindow(myID); }
...
static void myZprReshape(int w,int h){ myZprManager->zprReshape(w,h);}
static void myZprMouse(int button, int state, int x, int y){ myZprManager->zprMouse(button, state, x, y); }
static void myZprMotion(int x, int y){ myZprManager->zprMotion(x,y);}
static void my_pick(GLint name){ myZprManager->_pick(name); }
static void myZprKeyboard(unsigned char key, int x, int y){ myZprManager->zprKeyboard(key, x,y);}
static void myZprKeyboardUp(unsigned char key, int x, int y){ myZprManager->zprKeyboardUp(key, x,y);}
static void myZprSpecial(int key, int x, int y){ myZprManager->zprSpecial(key, x,y);}
static void myZprSpecialUp(int key, int x, int y){ myZprManager->zprSpecialUp(key, x,y);}

static void myZprDisplay(){ myZprManager->zprDisplay();}
static void myZprIdle(){ myZprManager->zprIdle();}
static void myZprDrawGraphics(){ myZprManager->zprDrawGraphics();}
...
class zprInstance{ public:
    ...
    int nextGLUTName(){ return(myZprManager->nextGLUTName);}
    void advanceGLUTName(){ myZprManager->nextGLUTName++;}
    ...
    void processHits(GLint hits, GLuint buffer[]);
	  ...
	  void zprInstanceInit(int ZprID, int glutID, zprManager * manager, int _NROW, int _NCOL, int nb){
		...
		myBi = new SA<int>(3);
		(*myBi)[0]=0; (*myBi)[1]=1;  (*myBi)[2]=2; 
		selection  = NULL;
		myglut2d = NULL; myglut3d=NULL;
		myGraphics.clear(); console_position=0; console_string[0]='\0';
		myZprInstanceID = ZprID; myGLUTWindowID = glutID; myZprManager = manager;
		myWindowHeight = _NROW;
		myWindowWidth  = _NCOL;
		NRow = _NROW; NCol = _NCOL;
		NBand = nb;
		myScreenPositionX= 0; myScreenPositionY = 0;
        _F1 = false;
		_mouseX      = 0;
		_mouseY      = 0;
		_mouseLeft   = 0;
		...
		isPaused = false; forceUpdate = false; usleepN = 15000; usleepNupdate = usleepN;
		myLeft= FLT_MAX;
		myRight=FLT_MIN;
		myBottom=FLT_MAX;
		myTop=FLT_MIN;
...
public:
	...
	int isInverted;
	void mark(){ Update = true; hideMe = false;}
	glPlottable(){ myType = NULL; Update = false; isLabelled=false; isLinkd =false; myLinks.clear(); hideMe = false; forceUpdate = false;}
	glPlottable(int isLbld){ myType=NULL;  glPlottable(); isLabelled = isLbld; hideMe = false; forceUpdate = false;}
	void setLabel(int lab){	myLabel = lab; isLabelled=true;}
    virtual void drawMe() {}
	virtual void drawMe(int highlight){}
	void setDefault(){ hideMe = false; forceUpdate = false;
		isLabelled = false;
		Update = false;
		isLinkd = false;
		isInverted = false;
	...
	void addPointToBoundingBox( float x, float y, float z){
		...
		if(x<parentZprInstance->myLeft){ parentZprInstance->myLeft=x;}
		if(x>parentZprInstance->myRight){ parentZprInstance->myRight =x;}
		if(y<parentZprInstance->myBottom){ parentZprInstance->myBottom = y;}
		if(y>parentZprInstance->myTop){parentZprInstance->myTop = y;}
		if(z<parentZprInstance->myZNear){ parentZprInstance->myZNear = z;}
		if(z>parentZprInstance->myZFar){ parentZprInstance->myZFar = z;}
  ...
  void setRGB(float R, float G, float B){ rgb.x=R; rgb.y=G; rgb.z=B;}
  ...
  void initName(zprInstance * parent, int useName);
	void colorMe(){
		if(isLinkd){ 
			//printf("glPlottable::colorMe() (LINKED OBJECT)\n");
		}
		if( (this->isPicked()) || isLinkd || this->isInverted){// (this->isLinked())){ 
...
public:
	...
	glImage(){ Update = false; isClusteringImage = false;}
    glImage(  zprInstance * parent, myImg * img ){myParent = parent;
		initFrom(parent,img); Update = false; isClusteringImage = false;
        	}
...
class glSimpleRect: public glPlottable{ public:
	vec3 x;
	int myVal;
	int * myParam;
	int K1, K2;
...
public:
	...
	glSlider( zprInstance * _myZpr, int nMax, int * paramVal, int pixW, int blankW){
		...
		for(i=0; i<nItems; i++){
			...
			(*myRects)[i] = new glSimpleRect( myZpr, rgb.x, rgb.y, rgb.z, i, paramVal,  (float)(i*(pixWidth+blankWidth)), 0.,0., pixWidth, pixWidth);//&x, &y);
		}
	}
	void resetMe(){
		int i;
...
class glPoint: public glPlottable{public:
	SA<float> * p;
	int N;
	vec3 x;
...
class glLine: public glPlottable{ public:
...
public:
	...
	glBasicSphere(){ setRelativePosition = 0;}
