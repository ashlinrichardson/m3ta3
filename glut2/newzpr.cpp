...
using namespace std;
  ...
  zprManager * zprManager::Instance(int argc, char *argv[]){
	...
	void glImage::rebuffer(){
		...
		myBi = parentZprInstance->myBi;//new SA<int>(3);

		printf("glImage::rebuffer() %d %d %d \n", myBi->at(0), myBi->at(1), myBi->at(2));//image->bi[0],image->bi[1], image->bi[2]);
		//focus();
		int NRow = image->NRow; int NCol = image->NCol;
		SA< SA<float> * > * FB = image->getFloatBuffers();
		printf("glImage::rebuffer() %d %d %d \n", myBi->at(0), myBi->at(1), myBi->at(2));
		SA<float> * b1 = FB->at( myBi->at(0));//image->getBi(0));
		SA<float> * b2 = FB->at( myBi->at(1));//image->getBi(1));
		SA<float> * b3 = FB->at( myBi->at(2));//image->getBi(2));
		int i,j,k,ri;	
		k=0;
		for(i=0; i<NRow; i++){
			ri = NRow-i-1;
	...
	zprInstance * zprManager::newZprInstance(int NROW, int NCOL, int NBAND){
		...
		dprintf("zprManager::newZprInstance(%d,%d)",myWindowWidth, myWindowHeight);//WINDOWX, WINDOWY);
        
		zprInstance * ret = NULL;
        glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
        glutInitWindowSize(myWindowWidth, myWindowHeight);//WINDOWX, WINDOWY);
        ...
        ret = new zprInstance(nextZprInstanceID, newWindowID, this, NROW, NCOL,myDims);//WINDOWX, WINDOWY);
	...
	ret->myZprInstanceID = nextZprInstanceID;
        ...
        glutDisplayFunc(myZprDisplay);//display);
        glutSpecialFunc(myZprSpecial);//keyboard);
        glutSpecialUpFunc(myZprSpecialUp);//keyboard);
        glutKeyboardFunc(myZprKeyboard);//keyboard);
        glutKeyboardUpFunc(myZprKeyboardUp);//keyboard);

        glutIdleFunc(myZprIdle);//idle);
	...
	void zprInstance::getrgb(int & r, int & g, int & b){
		r = myBi->at(0);//myglut::myglut3d->curband[0];
		g = myBi->at(1);//myglut::myglut3d->curband[1];
		b = myBi->at(2);//myglut::myglut3d->curband[2];
	}
...
printf("Key down: %d\n", key);	
		...
		{		
			case 127:
/*			case 'a':	
						ZPR_ZOOM_MODE = ! ZPR_ZOOM_MODE;	if(ZPR_ZOOM_MODE){ ZPR_PAN_MODE = ZPR_ROTATE_MODE = false;}
						printf(ZPR_ZOOM_MODE?"ZOOM MODE ON":"ZOOM MODE OFF");
						display();
						break;
			case 's':	
						ZPR_PAN_MODE = !ZPR_PAN_MODE;		if(ZPR_PAN_MODE){ ZPR_ZOOM_MODE = ZPR_ROTATE_MODE = false;}
						printf(ZPR_PAN_MODE?"PAN MODE ON":"PAN MODE OFF");
						display();
						break;
			case 'd':
						ZPR_ROTATE_MODE = !ZPR_ROTATE_MODE;	if(ZPR_ROTATE_MODE){ ZPR_ZOOM_MODE = ZPR_PAN_MODE = false;}
						printf(ZPR_ROTATE_MODE?"ROTATE MODE ON":"ROTATE MODE OFF");
						display();
						break;
...
//should evolve this method to use a "selected item" a-la Latest-2 syncitium.
        ...
        if (_mouseMiddle || (_mouseLeft && _mouseRight))  //ZOOM 
        {
...
//http://www.codeproject.com/Articles/80923/The-OpenGL-and-GLUT-A-Powerful-Graphics-Library-an
    ...
    void zprInstance::processHits(GLint hits, GLuint buffer[]){  //std::set<GLint> myPickNames;
    ...
    void zprInstance::zprPick(GLdouble x, GLdouble y,GLdouble delX, GLdouble delY){
        ...
        if (this->selection){ this->selection();}                                 // Draw the scene in selection mode
        ...
        glMatrixMode(GL_MODELVIEW);
        //if( myglut3d){   myglut3d->myParent->pick(0); }
		return;
    }
    void glPlottable::initName(zprInstance * parent, int useName){
		...
		if(useName){ 
			...
			//cout << "glPlottable::initName(): myName=("<<myName<<"), nextGLUTName()=(" << parentZprInstance->nextGLUTName() <<endl;
		}
