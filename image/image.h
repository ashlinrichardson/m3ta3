...
using namespace std;
  ...
  class myImg{
		public: 
			SA<int> * myBi;
			int getBi(int i){ return myBi->at(i);}
			void assignProjection( SA<int> * myb){ myBi = myb;}
			myImg(){}
      ...
      void clear(){
        ...
        for(i=0; i<mySize(); i++){
          (*this)[i] =0.;
			...
			int mySize(){ return NRow*NCol*NBand;}
			void assignFrom( myImg * other){
				if((other->mySize())!=(this->mySize())){ dprintf("Error: myImg.h: tried to assign from array of different size.");}
				int i; 
			...
			void assignFrom( SA<float> * other){
				if((other->size())!=(this->mySize())){ dprintf("Error: myImg.h: tried to assign from array of different size.");}
				int i; 
				for(i=0; i<other->size(); i++){
					(*this)[i] = (*other)[i];
				}return;
			...
			myImg(char * filename, int nrow, int ncol, int nb){ init(filename,nrow,ncol,nb);}
			SA< SA<float> * > * getFloatBuffers(){return float_buffers;}
				int NRow, NCol, NBand;
			SA<float> dat;
				private:
